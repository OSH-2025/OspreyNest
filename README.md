...
|5.30|线下集体研讨|将LED灯连接至树莓派，尝试串联起来所有模块测试LED灯是否可受语音控制|将LED灯连接至树莓派，发现树莓派电压不稳，未能实现语音控制|
|6.3|线下集体研讨|集体研讨当前问题|目前可以完成语音识别网页和树莓派端的连接以及wasm解析指令，但尚未实现整体串联；汇总当前任务：完成整体串联，加入唤醒词（可选），docker（可选），部署大模型进行语音识别（可选）|
|6.6|线下集体讨论|确认当前进度，规划后期项目安排|目前对语音识别网页进行了优化，添加了唤醒词“小欧小欧”，并且不只可以看web上的文字来接受反馈，语音识别系统打开麦克风和扬声器即可与我们进行对话；初步规划了考试后到答辩前的安排|
|6.12|线下集体讨论|总结当前进度，明确答辩时间和报告截止时间，作出下一步项目安排|当前已经初步实现了项目的功能，增加了树莓派控制引脚的脚本，通过了LED灯的测试；计划考试后完成wasm模块解析指令方案和不用wasm模块方案的时间对比测试，并完成PPT和答辩准备|
|6.17|线下集体讨论|总结当前问题，作出下一步项目安排|针对网页端访问限制的问题进行相关讨论，提出可能的解决方案；分配任务，包括测试不同方案及测试项目整体性能的方案调研|
|6.20|线下集体讨论|研讨方案可行性，作出下一步项目安排|针对网页端访问限制的方案进行汇总讨论，挑选出最优方案推进：即使用一台电脑作为服务器，其他设备通过其IP及相应端口进行控制；针对该方案进行细化，完成相关任务|
|6.23|线下集体讨论|总结当前进度，初步准备期末答辩|初步确定答辩内容及形式，针对期中汇报的PPT制作和总体效果进行评估，提出改进建议|
|6.24|线下集体讨论|确认当前进度及选择提高内容，集中准备期末答辩|根据WASM和JavaScript的指令解析性能进行相关测试，并总结成图表作为答辩内容；分配LAB4的不同方向调研工作；初步选择部署大模型进行语音识别作为主要优化目标|
|6.27|线下集体讨论|改进项目内容，确定并分配期末答辩ppt制作任务|部署大模型辅助解析指令 , ppt制作任务分配: 1. 背景+整体架构: 刘艺璇  2. 浏览器端语音识别+指令传输和树莓派执行: 郭芮含 3. 指令解析c/wasm: 杨晶晶  4. 性能和可靠性评估: 王安琪 5. 未来扩展工作+总结: 付嘉瑞|
|6.28|线下集体讨论|录制演示视频，改进项目内容|合成汇总ppt内容，录制演示视频以辅助讲解；根据老师建议，对性能对比测试的研究进行深入|
|6.29|线下集体讨论|整合优化PPT与演讲稿，完善答辩材料|撰写演讲稿并按页次设计讲解逻辑;改进预演发现的问题;新增性能评估模块，补充WASM与JS的耗时对比图表|
