...
|May 30|Offline Group Discussion|Connect LED lights to the Raspberry Pi and attempt to connect all modules in series to test whether the LED lights can be voice-controlled.|When connecting the LED lights to the Raspberry Pi, it was found that the voltage of the Raspberry Pi was unstable, failing to achieve voice control. However, the connection between the local browser and the Raspberry Pi has been established, and the Raspberry Pi can receive content from the web page.|
|June 3|Group Discussion on Current Issues|Offline group discussion|
